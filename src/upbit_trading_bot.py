...
while True:
    ...
    print("-------------------- 투자금 지정 완료:", str(invest_balance) + "원", "--------------------")

    # 투자대상 선정(1시간봉 기준 2개의 거래량 1등)
    print("-------------------- 투자 대상 선정 시작 --------------------")
    target_coin = methods.get_top_coin_list("minute60", 1)[0]
    print("-------------------- 투자 대상 선정 완료:", target_coin, "--------------------")
    ...
    while True:
        ...
        if buy_sign:
            ...
            if now_my_balance >= min_buy_price * (1 + buy_fees):
                ...
                print("-------------------- 매수 성공:", str(buy_result["price"]), "--------------------")
            else:
                print("-------------------- 잔액부족 매수실패 --------------------")
        ...
        if methods.is_has_coin(upbit.get_balances(), target_coin):
            ...
            for coin in upbit.get_balances():
                ...
                if pyupbit.get_current_price(target_coin) >= target_price:
                    ...
                    print("-------------------- 코인을 모두 판매했습니다. 10초 뒤 대상 선정을 다시 시작합니다. --------------------")
                    is_break = True
                else:
                    print("-------------------- 수익률에 도달하지 못했습니다. 10초 뒤 시장상황 분석 로직을 다시 시작합니다. --------------------")
        else:
            print("-------------------- 코인을 구매하지 않았습니다. 10초 뒤 대상 선정을 다시 시작합니다. --------------------")
            is_break = True
        time.sleep(10)
        if is_break:
            break
